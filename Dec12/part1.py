...
def get_neighbours(point, height_map):
    ...
    for offset in offsets:
        ...
        if 0 <= neighbour_pos[0] < len(height_map) and \
           ...
           0 <= height_at(neighbour_pos, height_map) <= height+1:
           neighbours.append(neighbour_pos)

    return neighbours

 
def a_star(start_point, goal_point, height_map):
...
path = a_star(start_point, goal_point, height_map)
print(len(path)-1)
